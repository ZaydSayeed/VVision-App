...
- Create: `widget-extension-plugin/ios/EvaluVisionWidget/EvaluVisionWidget.swift`
- Create: `widget-extension-plugin/ios/EvaluVisionWidget/Info.plist`
...
```swift
// EvaluVisionWidget.swift — illustrative; adjust types/imports to match the
...
struct EvaluVisionProvider: TimelineProvider {
    ...
    func getSnapshot(in context: Context, completion: @escaping (EvaluVisionEntry) -> Void) {
        completion(EvaluVisionEntry(date: Date(), snapshot: nil))
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<EvaluVisionEntry>) -> Void) {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            if let snapshot = entry.snapshot {
                ...
                Text("Open EvaluVision to set up this widget")
            }
        }
        .padding()
    }
...
struct EvaluVisionWidget: Widget {
    ...
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: EvaluVisionProvider()) { entry in
            EvaluVisionWidgetView(entry: entry)
        }
        .configurationDisplayName("EvaluVision Today")
...
- [ ] Set `ios.appleTeamId` in `app.json` — `@bacons/apple-targets` needs your Apple Developer Team ID to sign the new widget extension target. Find it at developer.apple.com/account under Membership.
- [ ] Register a separate App ID for the widget extension's own bundle identifier (`com.velavision.caregiver.EvaluVisionWidget`, or whatever the config plugin ends up producing — confirm the exact suffix it generates) in the Apple Developer portal, in addition to the main app's existing App ID.
- [ ] Add the App Group (`group.com.velavision.caregiver.widget`) to BOTH the main app's App ID and the widget extension's new App ID in the Apple Developer portal — an app group must be enabled on every App ID that uses it, not just one.
- [ ] Expect the first EAS build after this merges to fail on code signing if any of the three items above aren't done first — this is a known, expected failure mode, not a bug in the code.
